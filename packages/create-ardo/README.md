...
The scaffold copies the app shell, home route, config files, and deployment workflow. `app/routes.ts`
is generated by the Ardo route plugin from the files in `app/routes/`; it is documented here so the
project tree matches a running scaffolded project.
