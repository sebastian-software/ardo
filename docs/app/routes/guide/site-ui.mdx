...
| `labels`         | `ArdoLabelsInput`                                                                  | Override default UI chrome strings and aria labels.                            |
...
Use `labels` for single-language sites that need non-English UI chrome without replacing every
default component. Ardo merges your overrides with English defaults, so you can set only the strings
you need.
...
<ArdoRoot
  ...
  labels={{
    ...
    search: {
      ...
      resultCount: (count) => `${count} ${count === 1 ? "Treffer" : "Treffer"}`,
    },
    toc: {
      label: "Auf dieser Seite",
      navLabel: "Inhaltsverzeichnis",
    },
    footer: {
      builtOn: "Erstellt am",
...
The label object covers search, sidebar collapse buttons, mobile menu buttons, breadcrumbs, footer
prefixes, copy buttons, callout title defaults, the skip link, and table-of-contents labels. Existing
single-purpose props still work: `headerProps.searchPlaceholder` overrides `labels.search.placeholder`,
and `tocLabel` overrides `labels.toc.label`.
