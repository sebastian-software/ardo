...
When using the `CodeBlock` component in `.tsx` route files (outside MDX), syntax highlighting is applied automatically at build time. The `ardo:codeblock-highlight` Vite plugin detects static `code` and `language` props and pre-renders Shiki HTML:

```tsx
import { CodeBlock } from "ardo/ui"

export default function MyPage() {
  return <CodeBlock code="const x: number = 42" language="typescript" />
}
```

This works out of the box — no extra configuration needed. If your code or language is dynamic, the component falls back to plain text rendering.
