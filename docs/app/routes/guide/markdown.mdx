...
The `CodeBlock` component can be used directly in `.tsx` route files. Code can be passed as children (recommended) or via the `code` prop:
...
Common indentation from template literals is stripped automatically, so you can indent naturally inside JSX. The `ardo:codeblock-highlight` Vite plugin pre-renders Shiki HTML at build time — no extra configuration needed.

The `code` prop is still supported for backwards compatibility:

```tsx
<CodeBlock code="const x = 42" language="typescript" />
```

If code or language are dynamic (from variables), the component falls back to plain text rendering.
