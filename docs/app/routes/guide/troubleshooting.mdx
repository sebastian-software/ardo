...
The `ardo:codeblock-highlight` plugin only processes `CodeBlock` usages with **static string** `code` and `language` props. If your props are dynamic (from variables, props, or expressions), highlighting won't be applied at build time.

For dynamic code, use the `highlightCode` utility from `ardo/vite` at build time:
